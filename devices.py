...
def close_window():
    ...
    #     key_step += 1
    # keyboard.send("enter")
    sp.Popen('taskkill /im telnet.exe /f')
...
def ping_test_terminal(d):
    d = int(d)
    d = d % 100 + 60
    result = ping('192.168.24.' + str(d), verbose=False)
    return str(result)


def ping_test_barrier(d):
    d = int(d)
    result = ping('192.168.24.' + str(d), verbose=False)
    return str(result)
...
def test_svrem():
    ...
    # time.sleep(1.5)
    # close_window()
    sp.Popen('taskkill /im svrem.exe /f')
...
def test_pglsvrem():
    ...
    # time.sleep(1.5)
    # close_window()
    sp.Popen('taskkill /im pglsvrem.exe /f')
...
# расклдка клавы по умолчанию английская
def layout_en():
    time.sleep(0.2)
